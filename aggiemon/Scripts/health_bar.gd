...
func _on_health_changed(_new_hp: int = 0):
	...
	# Color-code health bar based on HP percentage
	var hp_percent = (enemy.curr_hp / enemy.max_hp) * 100
	var fill_style = self.get_theme_stylebox("fill") as StyleBoxFlat
	if fill_style:
		if hp_percent > 60:
			fill_style.bg_color = Color(0, 1, 0, 1)  # Green
		elif hp_percent > 30:
			fill_style.bg_color = Color(1, 1, 0, 1)  # Yellow
		else:
			fill_style.bg_color = Color(1, 0, 0, 1)  # Red
